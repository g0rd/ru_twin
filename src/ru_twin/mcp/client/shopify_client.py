...
# Import ThirdPartyAgentGateway directly to avoid circular imports
from ru_twin.third_party_gateway import ThirdPartyAgentGateway

# Import MCPClient only for type checking to avoid circular imports
if TYPE_CHECKING:
...
# Define the class without inheritance first to allow for delayed MCPClient import
class ShopifyClient:
    """
    ...
    # This will be set to the actual MCPClient class when needed
    _mcp_client_class = None
    ...
    def __init__(
        ...
        # Set the MCPClient class reference if not already set
        if ShopifyClient._mcp_client_class is None:
            ShopifyClient._mcp_client_class = MCPClient
        
        # Create a base URL for the Shopify store
        base_url = f"https://{shop_url}"
        
        # Initialize the parent MCPClient with a name and config
        self._mcp_client = ShopifyClient._mcp_client_class(
            name=f"shopify_{shop_url.split('.')[0]}",
            config=ClientConfig(
                base_url=base_url,
                timeout=60.0,
                use_sse=False
        ...
        # Set up the rest of the Shopify client
        self.shop_url = shop_url
        self.access_token = access_token
        self.api_version = api_version
        self.logger = logging.getLogger(__name__)
        self.session = None
        self.gateway = gateway  # Store the gateway for future use
        
        # GraphQL endpoint
        self.endpoint = f"{base_url}/admin/api/{self.api_version}/graphql.json"
        
        # Headers for GraphQL requests
        self.headers = {
            "Content-Type": "application/json",
            "X-Shopify-Access-Token": self.access_token,
            "User-Agent": f"RuTwin-Shopify-Client/{self.api_version}"
        }
    ...
    async def initialize(self):
        ...
        This method initializes both the parent MCPClient and the Shopify-specific session.
        ...
        try:
            ...
            if hasattr(self._mcp_client, 'initialize') and callable(self._mcp_client.initialize):
                await self._mcp_client.initialize()
            
            # Set up the Shopify-specific session
            if not hasattr(self, 'session') or self.session is None or self.session.closed:
                self.session = aiohttp.ClientSession(
                    ...
                    timeout=aiohttp.ClientTimeout(total=60.0)  # Default timeout of 60 seconds
                )
                self.logger.info("Shopify client session initialized")
            
            self._initialized = True
            return self
            
        except Exception as e:
            self.logger.error(f"Failed to initialize Shopify client: {str(e)}", exc_info=True)
            self._initialized = False
            ...
            raise RuntimeError(f"Failed to initialize Shopify client: {str(e)}") from e

    async def cleanup(self):
        """
        Clean up resources.
        
        This method cleans up both the Shopify-specific session and the MCP client resources.
        """
        errors = []
        
        try:
            # Clean up the Shopify session
            if hasattr(self, 'session') and self.session and not self.session.closed:
                try:
                    await self.session.close()
                    self.session = None
                    self.logger.info("Shopify client session closed")
                except Exception as e:
                    error_msg = f"Error closing Shopify session: {str(e)}"
                    self.logger.error(error_msg, exc_info=True)
                    errors.append(error_msg)
            
            # Clean up MCP client resources
            if hasattr(self, '_mcp_client') and self._mcp_client:
                if hasattr(self._mcp_client, 'cleanup') and callable(self._mcp_client.cleanup):
                    try:
                        await self._mcp_client.cleanup()
                        self.logger.info("MCP client cleaned up")
                    except Exception as e:
                        error_msg = f"Error cleaning up MCP client: {str(e)}"
                        self.logger.error(error_msg, exc_info=True)
                        errors.append(error_msg)
            
            self._initialized = False
            
            # If there were any errors during cleanup, raise an exception
            if errors:
    ...
    async def get_current_tool_names(self) -> List[str]:
        ...
        Note:
            If the MCP client doesn't implement get_available_tools, returns an empty list.
        """
        try:
            ...
            # Check if _mcp_client exists and has get_available_tools method
            if not hasattr(self, '_mcp_client') or not self._mcp_client:
                self.logger.warning("MCP client not initialized")
                return []
                
            if not hasattr(self._mcp_client, 'get_available_tools') or not callable(self._mcp_client.get_available_tools):
                self.logger.warning("MCP client does not implement get_available_tools")
                return []
                
            # Use the MCP client's implementation
            tools = await self._mcp_client.get_available_tools()
            
            # Extract just the tool names if the response is a list of tool objects
            if tools and isinstance(tools, list):
                if len(tools) > 0 and isinstance(tools[0], dict):
                    return [tool.get('name') for tool in tools if tool and tool.get('name')]
                # If it's a list of strings, return as is
                elif all(isinstance(tool, str) for tool in tools):
                    return tools
                    
            return tools or []
            
        except Exception as e:
            self.logger.error(f"Error getting tool names: {str(e)}", exc_info=True)
            return []
